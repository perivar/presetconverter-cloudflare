...
export interface FrequencyPostParameters {
  ...
  // Version 1 specific
  masterBypass?: number;
  // Spectrum params (COMP for v1, CONT for v2)
...
export class SteinbergFrequency extends SteinbergVstPreset {
  ...
  constructor(version: 1 | 2 = 1) {
    ...
    if (this.version === 1) {
      this.setCompChunkDataHeader(19728);
    } else {
  ...
  private initFrequencyBandParameters(bandNum: number): void {
    ...
    this.setNumberParameterWithIndex(
      ...
      116 + inc,
      this.version === 1 ? 100.0 * bandNum : bandNum === 1 ? 25.0 : 100.0
    );
    this.setNumberParameterWithIndex(
    ...
    this.setNumberParameterWithIndex(
      ...
      276 + inc,
      this.version === 1 ? 25.0 : bandNum === 1 ? 25.0 : 100.0
    );
    this.setNumberParameterWithIndex(
  ...
  private initFrequencyPostParameters(): void {
    if (this.version === 1) {
      ...
      this.setNumberParameterWithIndex(
        `${VstPreset.CHUNK_COMP}spectrumonoff`,
        1007,
        1.0
      );
      this.setNumberParameterWithIndex(
        `${VstPreset.CHUNK_COMP}spectrum2ChMode`,
        1008,
        0.0
      );
      this.setNumberParameterWithIndex(
        `${VstPreset.CHUNK_COMP}spectrumintegrate`,
        1010,
        40.0
      );
      this.setNumberParameterWithIndex(
        `${VstPreset.CHUNK_COMP}spectrumPHonoff`,
        1011,
        1.0
      );
      this.setNumberParameterWithIndex(
        `${VstPreset.CHUNK_COMP}spectrumslope`,
        1012,
        0.0
      );
      this.setNumberParameterWithIndex(
        `${VstPreset.CHUNK_COMP}draweq`,
        1013,
        1.0
      );
      this.setNumberParameterWithIndex(
        `${VstPreset.CHUNK_COMP}draweqfilled`,
        1014,
        1.0
      );
      this.setNumberParameterWithIndex(
        `${VstPreset.CHUNK_COMP}spectrumbargraph`,
        1015,
        0.0
      );
      this.setNumberParameterWithIndex(
        `${VstPreset.CHUNK_COMP}showPianoRoll`,
        1019,
        1.0
      );
      this.setNumberParameterWithIndex(
        `${VstPreset.CHUNK_COMP}transparency`,
        1020,
        0.3
      );
      this.setNumberParameterWithIndex(
  ...
  private readFrequencyPostParameters(): FrequencyPostParameters {
    const chunk =
      this.version === 1 ? VstPreset.CHUNK_COMP : VstPreset.CHUNK_CONT;
    return {
      equalizerAbypass:
      ...
      // Version 1 specific
      masterBypass:
        this.getNumberParameter(`${VstPreset.CHUNK_COMP}bypass`) || 0,
      // Spectrum params
      spectrumOnOff: this.getNumberParameter(`${chunk}spectrumonoff`) || 0,
      spectrum2ChMode: this.getNumberParameter(`${chunk}spectrum2ChMode`) || 0,
      spectrumIntegrate:
        this.getNumberParameter(`${chunk}spectrumintegrate`) || 0,
      spectrumPHOnOff: this.getNumberParameter(`${chunk}spectrumPHonoff`) || 0,
      spectrumSlope: this.getNumberParameter(`${chunk}spectrumslope`) || 0,
      drawEQ: this.getNumberParameter(`${chunk}draweq`) || 0,
      drawEQFilled: this.getNumberParameter(`${chunk}draweqfilled`) || 0,
      spectrumBarGraph:
        this.getNumberParameter(`${chunk}spectrumbargraph`) || 0,
      showPianoRoll: this.getNumberParameter(`${chunk}showPianoRoll`) || 0,
      transparency: this.getNumberParameter(`${chunk}transparency`) || 0,
      // Version 2 specific
      AutoListen:
        this.getNumberParameter(`${VstPreset.CHUNK_COMP}AutoListen`) || 0,
  ...
  public toString(): string {
    ...
    if (this.postParams) {
      if (this.version === 1) {
        ...
        lines.push(
          `autoGainOutputValue: ${this.postParams.autoGainOutputValue}`
        );
      } else {
        lines.push(`equalizerAoutput: ${this.postParams.equalizerAoutput}`);
        lines.push(`equalizerAbypass: ${this.postParams.equalizerAbypass}`);
      ...
      // Spectrum params (common to both versions)
